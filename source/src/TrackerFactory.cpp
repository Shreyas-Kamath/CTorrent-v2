...
std::shared_ptr<BaseTracker> make_tracker(boost::asio::io_context& ioc, const std::string_view url, const std::array<unsigned char, 20>& info_hash) {
    ...
    if (url.starts_with("https://")) return std::make_shared<HttpsTracker>(ioc, url, info_hash);
    else if (url.starts_with("udp://")) return std::make_shared<UdpTracker>(ioc, url, info_hash);
